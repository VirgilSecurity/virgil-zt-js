...
class ZtMiddleware {
	...
	constructor(keyType: KeyPairType, loginPath: string, encoding: BufferEncoding = 'base64') {
		ZtMiddleware.initializeCryptoModule().then(() => {
	...
	private encrypt(data: string): string {
		return this.virgilCrypto.signThenEncrypt(data, this.encryptKeys.privateKey, [this.encryptKeys.publicKey, this.frontendPublicKey]).toString(this.encryptEncoding);
	}

	private decrypt(data: string): string {
		return this.virgilCrypto.decryptThenVerify(data, this.encryptKeys.privateKey, [this.encryptKeys.publicKey, this.frontendPublicKey]).toString('utf-8');
	}
	...
	private loginFlow(key: string, res: Response): void {
		...
		this.setKey(key);
		res.send({key: this.virgilCrypto.exportPublicKey(this.encryptKeys.publicKey).toString(this.encryptEncoding)});
	}
